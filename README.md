...
- 对于反取证，Macbook和iPhone，有没有哪些缺陷（或不足） ？
    - 缺陷点是什么？
        ...
        - 缺陷 3 : 目前也不能秒防 FBI、CIA 通过无死角的海量监控 ，在视频中，抓拍到你解锁的手机按键动作，从而动作反推。而且全程AI自动定位，自动识别。
        
    - 🔥 【 对缺陷 1 】  ： 未来有无改进空间？
        - 那肯定有的。苹果应该允许用户设置 多组 “销毁Pin码” 。
    ...
    - 🔥 【 对缺陷 2 】 ：未来有无改进空间？
    	- 那肯定是有的。苹果应该允许用户，通过Siri语音，锁屏身边所有设备（以及能够通过Siri一同锁定，同Apple ID下的所有异地设备）。
    ...
    - 🔥 【 对缺陷 3 】 ：未来有无改进空间？
    	- 那肯定是有的。苹果应该允许用户，单独设置 “冷启动解锁密码” 。
        	- 目前苹果的 “冷启动” 锁屏密码 和 “启动后的” 锁屏密码 ，是相同的。但人们在使用手机过程中，99.99%的时间，都是在后者情况下解锁手机。所以，路面上的监控摄像头99.99%的概率，也只能抓拍到后者解锁密码。如果此时能单独为前者设置解锁密码，并在危急时刻，将手机快速进入冷启动模式，则立马上警摄像头方全部抓瞎 🤣🤣🤣
        	- 目前的 苹果设备，只在如下情况，会进入冷启动模式：
            	- 重启设备
            	- 超过2天 不解锁手机
            	- 超过2天 断网
            	- 快速按5下电源键
        - 苹果公司，只需要再支持：
            - 用户可以设置：语音呼唤Siri，通过声纹识别，批量让自己账号下的设备 进入冷启动锁屏模式
            - 用户可以设置： 解锁Pin码 + 扫脸 “特定表情” 解锁 ，两者必须 同时都验证通过，才能解锁。也就是说，警方哪怕知道了 冷启动解锁Pin码，也需要用户配合做出特定表情。如果用户做出的是 ，用户自己预先设置的 “销毁数据” 的 扫脸专属表情。那警方立马抓瞎 🤣🤣 想想都觉得酸爽 
      	- 所以，什么才叫 系统级 “原生支持” 反取证？？？用各种第三方垃圾，散装出来的纯软件方案，可能跟人家原生的比？

    - 🔥 最后：对于上述所有情况，警方能否不接招，直接暴力破解硬盘，从而解锁？ 比如在尝试输入Pin码前，先全盘景象，能不能依靠景象无限循环进行Pin尝试（取证尝试）？
